...
exec('yarn --version', (versionError, versionStdout, versionStderr) => {
    ...
    const auditCommand = yarnVersion.startsWith('4.')
        ? `yarn npm audit --json ${flagsString}`
        : `yarn audit --json ${flagsString}`;
        ...
        if (yarnVersion.startsWith('1.')) {
            ...
            lines.forEach((line) => {
                if (line) {
                    ...
                    if (
                        advisoryId &&
                        !advisoryToFilter.some((advisory) => advisoryId?.includes(advisory))
                    ) {
                        hasAdvisories = true;
